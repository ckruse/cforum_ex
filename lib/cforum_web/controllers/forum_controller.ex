defmodule CforumWeb.ForumController do
  ...
  def index(conn, _params) do
    {latest_threads, newest_thread} =
      Enum.reduce(conn.assigns[:visible_forums], {%{}, nil}, fn f, {acc, latest} ->
        {_, threads} =
          Threads.list_threads(f, nil, conn.assigns[:current_user], limit: 3, order: "newest-first", sticky: nil)
        ...
        threads =
          Enum.map(threads, fn t ->
            ...
            first_unread =
              Enum.filter(t.sorted_messages, fn m -> m.attribs[:is_read] != true end)

defmodule Cforum.Sortable do
  ...
  def sort(query, conn, valid_fields, replacements \\ [], defaults \\ [dir: :asc]) do
    ...
    {set_cookie_col, sort_col} = if CforumWeb.Helpers.blank?(conn.params["sort"]) do
      {false, conn.cookies[cookie_key_col] || Atom.to_string(List.first(valid_fields))}
    else
      {true, conn.params["sort"]}
    end

    {set_cookie_dir, sort_dir} = if CforumWeb.Helpers.blank?(conn.params["dir"]) do
      {false, conn.cookies[cookie_key_dir] || defaults[:dir]}
    else
      {true, conn.params["dir"]}
    end

    sort_dir = if sort_dir != "asc" && sort_dir != "desc",
      do: :asc,

defmodule Cforum.MessageView do
  use Cforum.Web, :view
  ...
  def message_tree(conn, thread, parent, messages, opts \\ [show_icons: true]) do
    parts = Enum.map(messages, fn(msg) ->
      # TODO classes
      subtree = if Cforum.Helpers.blank?(msg.messages), do: "", else: message_tree(conn, thread, msg, msg.messages)
      [{:safe, "<li>"} |
       [ render(Cforum.MessageView, "header.html", Keyword.merge([conn: conn, thread: thread, parent: parent, message: msg], opts)) |
         [subtree | {:safe, "</li>"}] ] ]
    end)

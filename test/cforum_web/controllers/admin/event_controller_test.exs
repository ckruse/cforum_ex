defmodule CforumWeb.Admin.EventControllerTest do
  ...
  describe "create event" do
    ...
    test "redirects to edit when data is valid", %{conn: conn} do
      ...
      conn = post(conn, Routes.admin_event_path(conn, :create), event: params)

      assert %{id: id} = cf_redirected_params(conn)
      assert redirected_to(conn) == Routes.admin_event_path(conn, :edit, id)

      conn = get(conn, Routes.admin_event_path(conn, :edit, id))
      assert html_response(conn, 200) =~ gettext("edit event %{name}", name: params[:name])
    end

    test "renders errors when data is invalid", %{conn: conn} do
      conn = post(conn, Routes.admin_event_path(conn, :create), event: %{})
      assert html_response(conn, 200) =~ gettext("new event")
    end
  end
  ...
  describe "update event" do
    ...
    test "redirects when data is valid", %{conn: conn, event: event} do
      conn = put(conn, Routes.admin_event_path(conn, :update, event), event: %{name: "foo bar"})
      assert redirected_to(conn) == Routes.admin_event_path(conn, :edit, event)
    ...
    test "renders errors when data is invalid", %{conn: conn, event: event} do
      conn = put(conn, Routes.admin_event_path(conn, :update, event), event: %{name: ""})
      assert html_response(conn, 200) =~ gettext("edit event %{name}", name: event.name)
    end
  end
  ...
  describe "delete event" do
    ...
    test "deletes chosen event", %{conn: conn, event: event} do
      ...
      assert_error_sent(404, fn ->
        get(conn, Routes.admin_event_path(conn, :edit, event))
      end)
    end
  end
  ...
  describe "access rights" do
    test "anonymous isn't allowed to access", %{conn: conn} do
      assert_error_sent(403, fn -> get(conn, Routes.admin_event_path(conn, :index)) end)
    end
    ...
    test "admin is allowed", %{conn: conn} do
      ...
      conn =
        conn
        |> login(user)
        |> get(Routes.admin_event_path(conn, :index))
